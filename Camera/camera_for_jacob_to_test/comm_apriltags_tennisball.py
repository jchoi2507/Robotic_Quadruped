...
from thresholds import thresholdsOrange, thresholdsTennisBall, STOP_MAX
...
msgs = ['n','s','f','r','l','d','h'] # nothing, stop, forward, dance, h = move head and dance
...
while True:
    ...
    is_tennis_color, blobs = detection.is_color(img,[thresholdsTennisBall]) # add more thresholds imported inside [] if applicable

    # checking roundness of blob is hopefully more reliable than using find circle separately b/c then can find a circle object and a green object and say there is a tennis ball when they aren't the same thing
    # is there a way to use circle and compare location of centroid? this seems easier right now though
    # !!! could make part of blob is_color function and get all info...but what if don't need?
    max_roundness = 0.50 # docs say 1 is a perfect circle, but from testing a square is 1...so now 'higher' roundness is closer to 0 than 1
    min_convex = 0.40
    is_blob_circle = detection.is_blob_round(blobs,max_roundness) # only give one blob, assume only one is found !!! adjust if more than one
    is_blob_curved = detection.is_blob_convex(blobs,min_convex)
    if len(blobs) > 0:
        img.draw_circle(blobs[0].enclosing_circle())

    #is_circle = detection.is_circle(img)
